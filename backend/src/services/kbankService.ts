...
export const createCharge = async (req: KBankCreateChargeRequest): Promise<KBankChargeResult> => {
  ...
  try {
    ...
    throw new Error(
      `KBank Create Charge failed: ${errData?.description || errData?.errorDesc || error.message}`
    );
  }
};
 ...
 */
export const inquireCharge = async (chargeId: string, partnerTxId: string): Promise<KBankInquiryResult> => {
  if (!API_KEY || !MERCHANT_ID) {
  ...
  try {
    ...
    const isPaid = ['Success', 'PAID', 'success', 'paid', '00', 'approved'].includes(
      String(data.status || data.txnStatus || '').toLowerCase()
    );
    ...
    const errData = error.response?.data;
    throw new Error(
      `KBank Inquiry failed: ${errData?.description || errData?.errorDesc || error.message}`
    );
  }
};
