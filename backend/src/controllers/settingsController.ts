  ...
  try {
    ...
    const updateResult = await pool.query(
      `UPDATE settings 
       ...
       WHERE key = $5
       RETURNING *`,
      [stringValue, description, category, data_type, key]
    );
    ...
    if (updateResult.rows.length === 0) {
      ...
      const insertResult = await pool.query(
        `INSERT INTO settings (key, value, description, category, data_type)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`,
        [key, stringValue, defaultDescription, defaultCategory, defaultDataType]
      );
...
// Get price per 100g
export const getPricePer100g = async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      "SELECT value FROM settings WHERE key = 'price_per_100g'"
    );
...
export const updatePricePer100g = async (req: Request, res: Response) => {
  try {
