...
export const listDriveFolder = async (req: Request, res: Response) => {
  try {
    if (!isDriveConfigured()) {
      return res.status(503).json({
        success: false,
    ...
    const folderId = (req.query.folderId as string)?.trim() || getDefaultFolderId();
    if (!folderId) {
...
export const uploadDriveImage = async (req: Request, res: Response) => {
  try {
    if (!isDriveConfigured()) {
      return res.status(503).json({
        success: false,
