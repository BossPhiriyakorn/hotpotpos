...
export const getDashboardSummary = async (req, res) => {
    try {
        ...
        if (branchId && branchId !== 'all' && branchId !== '') {
            finalBranchId = parseInt(branchId);
        }
    ...
    `;
      ...
      WHERE DATE(created_at) >= $1 AND DATE(created_at) <= $2
        AND order_status != 'cancelled'
    `;
        const prevSummaryParams = [prevStart, prevEnd];
        if (finalBranchId !== null) {
    ...
    `;
      ...
      LIMIT 10
    `, [start, end]);
        const topProducts = topProductsResult.rows.map((product, index) => ({
            id: product.id,
            name: product.name,
...
export const getProductSales = async (req, res) => {
    ...
    `, params);
        const products = result.rows.map((product) => ({
            ...
            quantity: parseInt(product.quantity),
            total: parseFloat(product.total) || 0,
            trend: 'stable', // Can be calculated based on previous period
        }));
        res.json({
            success: true,
            data: products,
...
export const getOrdersForReports = async (req, res) => {
    try {
