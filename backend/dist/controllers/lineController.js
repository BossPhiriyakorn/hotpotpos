...
export const generateOrderQRCode = async (req, res) => {
    try {
        ...
        // Verify order exists
        const orderResult = await pool.query('SELECT id, queue_number, order_number FROM orders WHERE id = $1', [orderId]);
        ...
        await pool.query(`INSERT INTO line_notifications (order_id, line_user_id, notification_enabled)
       VALUES ($1, $2, false)
       ON CONFLICT (order_id, line_user_id) DO NOTHING`, [order.id, `temp_${token}`]);
...
export const connectLineToOrder = async (req, res) => {
    try {
        ...
        const orderResult = await pool.query(`SELECT id, queue_number, order_number 
       FROM orders 
       WHERE id = $1 
         AND DATE(created_at) = CURRENT_DATE
         AND order_status != 'cancelled'`, [orderId]);
        if (orderResult.rows.length === 0) {
        ...
        await pool.query(`INSERT INTO line_notifications (order_id, line_user_id, line_display_name, notification_enabled)
       VALUES ($1, $2, $3, true)
       ON CONFLICT (order_id, line_user_id) 
       DO UPDATE SET 
         line_display_name = $3,
         notification_enabled = true,
         updated_at = CURRENT_TIMESTAMP`, [order.id, lineUserId, displayName]);
        // Send confirmation message
        try {
            await sendOrderNotification(lineUserId, order.queue_number, 'queued');
        }
        catch (error) {
...
export const getOrderLineStatus = async (req, res) => {
    try {
        const { orderId } = req.params;
...
export const disableOrderNotification = async (req, res) => {
    try {
        const { orderId, lineUserId } = req.body;
        await pool.query(`UPDATE line_notifications 
       SET notification_enabled = false, updated_at = CURRENT_TIMESTAMP
       WHERE order_id = $1 AND line_user_id = $2`, [orderId, lineUserId]);
...
export const notifyOrderStatusChange = async (orderId, newStatus) => {
    try {
        // Get order details
        const orderResult = await pool.query('SELECT id, queue_number FROM orders WHERE id = $1', [orderId]);
        if (orderResult.rows.length === 0)
            return;
        const order = orderResult.rows[0];
        // Get all LINE users connected to this order
        const notificationsResult = await pool.query(`SELECT line_user_id 
       FROM line_notifications 
       WHERE order_id = $1 AND notification_enabled = true`, [orderId]);
        // Send notification to each connected user
        for (const notif of notificationsResult.rows) {
            try {
                await sendOrderNotification(notif.line_user_id, order.queue_number, newStatus);
            }
            catch (error) {
                console.error(`Failed to send notification to ${notif.line_user_id}:`, error);
            }
        }
    }
    catch (error) {
