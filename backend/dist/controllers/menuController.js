...
export const getAddons = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM addons WHERE is_active = true ORDER BY sort_order');
...
export const createAddon = async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO addons (name, price, image_url, description, is_special, sort_order, is_active)
...
export const updateAddon = async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE addons 
...
export const deleteAddon = async (req, res) => {
    try {
        ...
        const checkResult = await pool.query('SELECT COUNT(*) as count FROM order_addons WHERE addon_id = $1', [id]);
        if (parseInt(checkResult.rows[0].count) > 0) {
            // Soft delete instead of hard delete
            const result = await pool.query('UPDATE addons SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ success: false, error: 'Addon not found' });
            }
...
// Get all soups
export const getSoups = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM soups WHERE is_active = true ORDER BY sort_order');
        res.json({ success: true, data: result.rows });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
export const createSoup = async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO soups (id, name, image_url, is_spicy, is_special, sort_order, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [
            id.trim(),
            name.trim(),
...
export const updateSoup = async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE soups 
       SET name = COALESCE($1, name),
...
export const deleteSoup = async (req, res) => {
    try {
        ...
        const checkResult = await pool.query('SELECT COUNT(*) as count FROM orders WHERE soup_id = $1', [id]);
        if (parseInt(checkResult.rows[0].count) > 0) {
            // Soft delete instead of hard delete
            const result = await pool.query('UPDATE soups SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ success: false, error: 'Soup not found' });
            }
...
export const getSpiceLevels = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM spice_levels WHERE is_active = true ORDER BY sort_order');
        res.json({ success: true, data: result.rows });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
export const createSpiceLevel = async (req, res) => {
    try {
        ...
        if (!sort_order || sort_order === 0) {
            const maxSortResult = await pool.query('SELECT COALESCE(MAX(sort_order), 0) AS max_sort FROM spice_levels');
            finalSortOrder = parseInt(maxSortResult.rows[0].max_sort) + 1;
        }
        const result = await pool.query(`INSERT INTO spice_levels (id, name, price, sort_order, is_active)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [
            id,
            name,
...
export const updateSpiceLevel = async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE spice_levels 
       SET name = COALESCE($1, name),
           price = COALESCE($2, price),
           ...
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $5
       RETURNING *`, [name, price, sort_order, is_active, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Spice level not found' });
        }
        res.json({ success: true, data: result.rows[0] });
...
export const deleteSpiceLevel = async (req, res) => {
    try {
        ...
        const checkResult = await pool.query('SELECT COUNT(*) as count FROM orders WHERE spice_level_id = $1', [id]);
        if (parseInt(checkResult.rows[0].count) > 0) {
            // Soft delete instead of hard delete
            const result = await pool.query('UPDATE spice_levels SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *', [id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ success: false, error: 'Spice level not found' });
            }
