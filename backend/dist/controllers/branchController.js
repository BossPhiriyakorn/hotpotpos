...
export const createBranch = async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO branches (name, code, address, phone, is_active)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [
            name.trim(),
            code ? code.trim() : null,
...
export const updateBranch = async (req, res) => {
    try {
        ...
        if (name && name.trim() !== '') {
            const nameCheck = await pool.query('SELECT id FROM branches WHERE name = $1 AND id != $2 AND is_active = true', [name.trim(), id]);
            if (nameCheck.rows.length > 0) {
                return res.status(400).json({
                    success: false,
                    error: 'Branch name already exists',
        ...
        if (code && code.trim() !== '') {
            const codeCheck = await pool.query('SELECT id FROM branches WHERE code = $1 AND id != $2 AND is_active = true', [code.trim(), id]);
            if (codeCheck.rows.length > 0) {
                return res.status(400).json({
                    success: false,
                    error: 'Branch code already exists',
        ...
        const result = await pool.query(`UPDATE branches 
...
export const deleteBranch = async (req, res) => {
    try {
        ...
        const ordersCheck = await pool.query('SELECT COUNT(*) as count FROM orders WHERE branch_id = $1', [id]);
        const orderCount = parseInt(ordersCheck.rows[0].count);
        // Check if branch has users
        const usersCheck = await pool.query('SELECT COUNT(*) as count FROM users WHERE branch_id = $1', [id]);
