...
export const createOrder = async (req, res) => {
    ...
    try {
        ...
        const { weight_grams, price_per_100g, base_price, soup_id, spice_level_id, addons, dining_location, table_number, cooking_style, note, payment_method, payment_reference, branch_id, // Get branch_id from request body (for kiosk)
         } = req.body;
        // Get branch_id from authenticated user if not provided in body (for CMS/API)
        let finalBranchId = branch_id;
        if (!finalBranchId && req.user) {
            const userResult = await client.query('SELECT branch_id FROM users WHERE id = $1', [req.user.id]);
        ...
        if (!finalBranchId) {
            const defaultBranchResult = await client.query('SELECT id FROM branches WHERE code = $1 AND is_active = true LIMIT 1', ['MAIN']);
        ...
        if (addons && Array.isArray(addons)) {
            for (const addon of addons) {
                ...
                if (!addonPrice) {
                    const addonResult = await client.query('SELECT price FROM addons WHERE id = $1', [addonId]);
        ...
        let spicePrice = 0;
        if (spice_level_id) {
            const spiceResult = await client.query('SELECT price FROM spice_levels WHERE id = $1', [spice_level_id]);
      ...
      RETURNING *`, [
        ...
        if (addons && Array.isArray(addons) && addons.length > 0) {
            for (const addon of addons) {
                ...
                await client.query(`INSERT INTO order_addons (order_id, addon_id, quantity, unit_price, total_price)
           VALUES ($1, $2, $3, $4, $5)`, [
                    order.id,
                    addonId,
                    quantity,
        ...
        await client.query(`INSERT INTO kitchen_order_status (order_id, status)
       VALUES ($1, 'queued')`, [order.id]);
        ...
        const fullOrderResult = await client.query(`SELECT 
        o.*,
        s.name AS soup_name,
        sl.name AS spice_level_name
      FROM orders o
...
export const getNextQueueNumber = async (req, res) => {
    try {
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        const result = await pool.query(`SELECT COALESCE(MAX(queue_number), 0) + 1 AS next_queue
       FROM orders
       WHERE created_at >= $1 AND created_at < $1 + INTERVAL '1 day'`, [todayStart]);
        res.json({
            success: true,
