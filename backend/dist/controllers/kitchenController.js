...
export const updateKitchenOrderStatus = async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query(`INSERT INTO kitchen_order_status (order_id, status, changed_by)
         VALUES ($1, $2, $3)`, [id, status, req.user?.id || null]);
         ...
         SET order_status = $1, updated_at = CURRENT_TIMESTAMP`;
            ...
            try {
                ...
                if (notificationStatus) {
                    await notifyOrderStatusChange(id, notificationStatus);
                }
            }
            catch (error) {
...
export const getKitchenStats = async (req, res) => {
    try {
        ...
        if (user && user.userType !== 'admin') {
            const userResult = await pool.query('SELECT branch_id FROM users WHERE id = $1 AND is_active = true', [user.id]);
