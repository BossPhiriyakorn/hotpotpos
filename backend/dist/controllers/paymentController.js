...
import { createCharge, inquireCharge, verifyWebhookSignature, getKBankConfig, } from '../services/kbankService.js';
// ---- Create KBank Charge ---------------------------------------------------
export const createKBankCharge = async (req, res) => {
    ...
    try {
        ...
        // ดึงข้อมูลออเดอร์
        const orderResult = await client.query('SELECT id, total_price, order_status, payment_status, kbank_charge_id FROM orders WHERE id = $1', [orderId]);
        ...
        if (order.kbank_charge_id) {
            const existingCharge = await client.query(`SELECT * FROM kbank_charges WHERE charge_id = $1 AND status = 'pending' AND expires_at > NOW()`, [order.kbank_charge_id]);
        ...
        await client.query(`INSERT INTO kbank_charges (
       ...
       ON CONFLICT (charge_id) DO UPDATE SET
        ...
        await client.query(`UPDATE orders SET kbank_charge_id = $1, payment_method = 'KBANK_QR', updated_at = CURRENT_TIMESTAMP WHERE id = $2`, [chargeResult.chargeId, order.id]);
        res.json({
            success: true,
            data: {
...
export const inquireKBankCharge = async (req, res) => {
    const client = await pool.connect();
    try {
        const { orderId } = req.params;
        const chargeRow = await client.query(`SELECT kc.* FROM kbank_charges kc
       JOIN orders o ON o.id = kc.order_id
       WHERE kc.order_id = $1
       ORDER BY kc.created_at DESC LIMIT 1`, [orderId]);
        ...
        if (inquiryResult.status === 'paid') {
            ...
            await client.query(`UPDATE kbank_charges SET status = 'paid', kbank_payment_id = $1,
         paid_at = $2, raw_webhook = $3, updated_at = CURRENT_TIMESTAMP
         WHERE id = $4`, [inquiryResult.kbankPaymentId, inquiryResult.paidAt, JSON.stringify(inquiryResult.rawResponse), charge.id]);
            await client.query(`UPDATE orders SET payment_status = 'paid', kbank_payment_id = $1,
         paid_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
         WHERE id = $2`, [inquiryResult.kbankPaymentId, orderId]);
...
export const kbankWebhook = async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        const chargeRow = await client.query('SELECT * FROM kbank_charges WHERE charge_id = $1', [chargeId]);
        if (chargeRow.rows.length === 0) {
        ...
        if (isPaid && charge.status !== 'paid') {
         ...
         WHERE charge_id = $3`, [kbankPaymentId, JSON.stringify(payload), chargeId]);
            await client.query(`UPDATE orders SET payment_status = 'paid', kbank_payment_id = $1,
         payment_reference = $2, paid_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
         WHERE id = $3`, [kbankPaymentId, chargeId, charge.order_id]);
            await client.query('COMMIT');
            console.log(`[KBank Webhook] ✅ Order ${charge.order_id} marked as PAID`);
        }
        else {
            // บันทึก webhook ดิบแม้ยังไม่ paid
            await client.query(`UPDATE kbank_charges SET raw_webhook = $1, status = $2, updated_at = CURRENT_TIMESTAMP
         WHERE charge_id = $3`, [JSON.stringify(payload), txnStatus || charge.status, chargeId]);
        }
...
export const getPaymentConfig = async (_req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT value FROM settings WHERE key = 'payment_mode'`);
